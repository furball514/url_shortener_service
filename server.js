...
Router.get('/url_does_not_exist', ctx => {
  ctx.body =
    '<h1>The URL you entered does not exist</h1><br><a href="https://ee.glitch.me">Click here to return to homepage</a>';
}); //page to show if input url doesn't exist
...
async function handleForm(ctx) {
  ...
  if (exist(pass.body.replace(/"/g, ''))) {
    //if input url(without quotes) exists then pass to shortened as param
...
async function shortened(ctx) {
  //search db if url already entered
...
async function red(ctx) {
  //redirect to long link from short link
